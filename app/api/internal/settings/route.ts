...
import { getDeveloperById, updateDeveloperSettings } from '@/lib/db/developers'
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      success: true,
      data: { webhookUrl: developer.webhookUrl, dripRate: developer.dripRate },
    })
  } catch (err) {
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (dripRate !== undefined && (dripRate < 1 || dripRate > 1000 || !Number.isInteger(dripRate))) {
      return NextResponse.json(
        { success: false, error: 'Drip rate must be an integer between 1 and 1000', code: 'INVALID_DRIP_RATE' },
        { status: 422 }
      )
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      return NextResponse.json({
        success: res.ok,
        data: { statusCode: res.status, ok: res.ok },
      ...
      return NextResponse.json({
        success: false,
        error: isTimeout ? 'Webhook timed out after 10s' : 'Webhook unreachable',
      })
    }
  } catch (err) {
